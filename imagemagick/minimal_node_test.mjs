import { initializeImageMagick, ImageMagick, MagickFormat, MagickReadSettings } from '@imagemagick/magick-wasm';
import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';
import { createRequire } from 'module';
import { Buffer } from 'buffer'; // Import Buffer

// Helper to get __dirname in ES modules and a require function
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const require = createRequire(import.meta.url); // Create a require function

// 1x1 black PNG (from prompt)
const base64Png = "";
const PngData = new Uint8Array(Buffer.from(base64Png, 'base64'));

// Polyfill TextEncoder and TextDecoder for the WASM module in Node.js
import { TextEncoder, TextDecoder } from 'util';
global.TextEncoder = TextEncoder;
global.TextDecoder = TextDecoder;


async function runTest() {
  console.log('Starting minimal Node.js test for ImageMagick WASM...');

  try {
    // 1. Read the WASM file
    const wasmPath = require.resolve('@imagemagick/magick-wasm/magick.wasm');
    const wasmBytes = fs.readFileSync(wasmPath);
    console.log(`WASM file loaded from: ${wasmPath}, size: ${wasmBytes.length} bytes`);

    // 2. Initialize ImageMagick
    await initializeImageMagick(wasmBytes);
    console.log('ImageMagick initialized successfully.');

    // 3. Prepare read settings
    const readSettings = new MagickReadSettings();
    readSettings.format = MagickFormat.Png;
    console.log('Read settings configured for PNG.');

    // 4. Attempt to read the PNG data
    console.log('Attempting ImageMagick.read() with PngData...');
    ImageMagick.read(PngData, readSettings, (image) => {
      console.log('ImageMagick.read() callback executed.');
      console.log(`Image details: ${image.width}x${image.height}, format: ${image.format}`);

      // 5. Attempt a simple operation (e.g., convert to JPEG)
      console.log('Attempting image.write() to JPEG format...');
      image.write(MagickFormat.Jpeg, (outputData) => {
        if (outputData && outputData.length > 0) {
          console.log(`Successfully converted to JPEG, output size: ${outputData.length} bytes.`);
          console.log('Minimal test PASSED!');
        } else {
          console.error('Minimal test FAILED: Conversion to JPEG resulted in empty data.');
        }
      });
    });
  } catch (error) {
    console.error('Minimal test FAILED with error:');
    console.error(error);
  }
}

runTest();
