...
test.describe('flatbuffers handler', () => {
    ...
    test('should display real fixture (.fbs)', async ({ page }) => {
        const filePath = path.resolve(__dirname, '../example/monster.fbs');
    ...
    test('should display real fixture (.bin)', async ({ page }) => {
        const filePath = path.resolve(__dirname, '../example/monster.bin');
        const content = await fs.readFile(filePath);
        const iframe = await runHandlerTest(page, {
            handler: 'flatbuffers',
            file: {
    ...
    test('should display real fixture (.bfbs)', async ({ page }) => {
        const filePath = path.resolve(__dirname, '../example/monster.bfbs');
        const content = await fs.readFile(filePath);
        const iframe = await runHandlerTest(page, {
            handler: 'flatbuffers',
            file: {
    ...
    test('should decode real fixture (.bin) with uploaded schema (.bfbs)', async ({ page }) => {
        const binPath = path.resolve(__dirname, '../example/monster.bin');
        const bfbsPath = path.resolve(__dirname, '../example/monster.bfbs');
