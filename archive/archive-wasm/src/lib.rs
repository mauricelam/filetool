...
#[wasm_bindgen]
pub fn extract_entry(data: &[u8], entry_name: &str) -> Result<Vec<u8>, JsValue> {
    // Try ZIP
    if let Ok(mut zip) = zip::ZipArchive::new(Cursor::new(data)) {
        if let Ok(mut file) = zip.by_name(entry_name) {
            let mut extracted = Vec::new();
            file.read_to_end(&mut extracted).map_err(|e| JsValue::from_str(&format!("Zip extraction error: {}", e)))?;
            return Ok(extracted);
        }
    }

    // Try 7z
    if let Ok(mut sevenz) = sevenz_rust2::ArchiveReader::new(Cursor::new(data), sevenz_rust2::Password::empty()) {
        let data = sevenz.read_file(entry_name).map_err(|e| JsValue::from_str(&format!("7z extraction error: {}", e)))?;
        return Ok(data);
    }
    ...
    while let Ok(mut reader) = cpio::NewcReader::new(cursor) {
        ...
        if entry.name() == entry_name {
            let mut extracted = Vec::new();
            reader.read_to_end(&mut extracted).map_err(|e| JsValue::from_str(&format!("Cpio extraction error: {}", e)))?;
            return Ok(extracted);
        }
        if let Ok(next_cursor) = reader.finish() {
            cursor = next_cursor;
            ...
            for entry in entries_iter {
                if let Ok(mut file) = entry {
                    if file.path().unwrap().to_string_lossy() == entry_name {
                        let mut extracted = Vec::new();
                        file.read_to_end(&mut extracted).map_err(|e| JsValue::from_str(&format!("Tar extraction error: {}", e)))?;
                        return Ok(extracted);
                    }
                }
            }
...
#[wasm_bindgen]
pub fn create_zip(files: JsValue) -> Result<Vec<u8>, JsValue> {
...
#[wasm_bindgen]
pub fn create_tar_gz(files: JsValue) -> Result<Vec<u8>, JsValue> {
    let files: Vec<FileToArchive> = serde_wasm_bindgen::from_value(files)?;
    let mut buf = Vec::new();
    {
        let enc = GzEncoder::new(Cursor::new(&mut buf), Compression::default());
