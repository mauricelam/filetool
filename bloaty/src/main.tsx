...
import { IconChartPie, IconFileText, IconLayout2 } from '@tabler/icons-react';
import { parseBloatyTsv, BloatyNode, formatBytes } from './utils';
...
function App() {
  ...
  const [dataSources, setDataSources] = useState<string[]>(['compileunits']);
  ...
  const [sizeType, setSizeType] = useState<'vmsize' | 'filesize'>('vmsize');
  ...
  useEffect(() => {
    ...
    workerRef.current.onmessage = (e) => {
      const { action, result, format, error } = e.data;
      if (action === 'result') {
        if (format === 'tsv') {
          setTsvResult(result);
        } else {
      ...
      <Container size="xl" py="md" style={{ height: '100vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <Tabs value={activeTab} onChange={setActiveTab} style={{ flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
          <Tabs.List mb="md">
            ...
            {/* <Tabs.Tab value="sunburst" leftSection={<IconChartPie size={16} />}>Sunburst</Tabs.Tab> */}
          </Tabs.List>
            ...
            <Paper p="md" withBorder shadow="xs" style={{ height: '100%', backgroundColor: '#1e1e1e' }}>
              <ScrollArea style={{ height: '100%' }}>
                <Text component="pre" data-testid="bloaty-output" style={{ margin: 0, fontFamily: 'monospace', fontSize: '12px', color: '#d4d4d4', whiteSpace: 'pre-wrap' }}>
                  {loading && !result ? 'Analyzing...' : result || 'No results yet.'}
                </Text>
              </ScrollArea>
            </Paper>
          </Tabs.Panel>
            ...
            <Paper p="md" withBorder shadow="xs" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
              <Group mb="md" justify="flex-end">
                <Switch
                  ...
                  checked={sizeType === 'filesize'}
                  onChange={(event) => setSizeType(event.currentTarget.checked ? 'filesize' : 'vmsize')}
                />
              </Group>
          ...
          {/* <Tabs.Panel value="sunburst" style={{ flexGrow: 1 }}>
            <Paper p="md" withBorder shadow="xs" style={{ height: '100%' }}>
              <div style={{ height: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                <Text>Sunburst view coming soon...</Text>
              </div>
            </Paper>
          </Tabs.Panel> */}
        </Tabs>
